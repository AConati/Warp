package edu.carleton.leeg2;

public class Main {

    public static void main(String[] args)
    {
        System.out.println("HELLO");
        System.out.println("      ooooooooooooooooo oooooooooooooooo oo oooooooooooooooooooooooooooo");
        System.out.println("   o\"                                                                   \"$o");
        System.out.println(" o\"                                                                       \"oo");
        System.out.println("o\"           ,   .            ,--.           .    ,---.         .           \"o");
        System.out.println("$            | . |            |    o         |      |           |             $");
        System.out.println("o            | ) ) ,-. , , ,  |-   . ;-. ,-. |-     |   ;-. . . |             \"o");
        System.out.println("$            |/|/  | | |/|/   |    | |   `-. |      |   |   | |               $");
        System.out.println(" o           ' '   `-' ' '    '    ' '   `-' `-'    '   '   `-| o             $");
        System.out.println(" \"o                                                         `-'             o\"");
        System.out.println("   \"oo                                                                	  o\"");
        System.out.println("      \"\" ooo o o   o o o o o o o o o o o   o  o   o o o        o o o oo\"");
        System.out.println("                $ $ \" \" \" \" \" \"  \"  \"  \" \" \" \" \" \"     \"\" \"\" \"");
        System.out.println("                o $");
        System.out.println("                 \"$o");
        System.out.println("                  \"$o");
        System.out.println("                    \"");
        System.out.println("");
        System.out.println("");
        System.out.println("");
        System.out.println("                   o$       o\"");
        System.out.println("                  o\"$     $\"$    ooo\"\"     o");
        System.out.println("                 $ o\"  oo\" $   o\"\" \"  oo$\"\" \"");
        System.out.println("                $   $oo\"  \"\"$$\"     \"\"\"");
        System.out.println("            oooo\"            o oo         oooooo");
        System.out.println("            \"o         ooo\"\"\"");
        System.out.println("            $$        \"                  o");
        System.out.println("           o                             \" \"\"\"\"o");
        System.out.println("      $o  o                                 $o");
        System.out.println("      \"$o\"                                  o$");
        System.out.println("      \"$\"                                    $o");
        System.out.println("   \"\"\"\"\"                                      \"o ooo");
        System.out.println("     ooo\" o$                 $$o             o$$\"   \"o");
        System.out.println("    \"   o             o$$    \"\"\"              \"\"     $");
        System.out.println("        \"o             \"$                      o  oo\"");
        System.out.println("         $               o$o  \"\"o             o$$\"\"");
        System.out.println("          $              $      $\"     oo$$\"  o$");
        System.out.println("       o$\"$$              \"ooo$\"\"oo$$$$$\"\"  o$\"");
        System.out.println("      $     oo        o oooo$$$$$$$$$$$\"   $\"");
        System.out.println("      $     $$   $o$$$$$$$$$$\"\"\" \"$$$\"   $\"");
        System.out.println("       $oooo$\"$o  \"\"\"\"\"$$$$$o     $\"   $\"                             ooooooo");
        System.out.println("               \"$ooooo     \"\"\"\"\"\"$  o$\"                      ooo$$\"\"\"\"      \"$o");
        System.out.println("                   \"\"\"$$oooooo$oo  \" \"$ooo         oooo$$\"$\"\"\"                $o");
        System.out.println("                     o$$ $ $ $o\"o$\"$ $ o$$$\"\"$$\"$\"\"                           o$");
        System.out.println("                 o o$$$\"       o o$$\"\"\"\"\" $o  $ \"$                       o oo\"\"");
        System.out.println("            ooo$\"\"$$$$$$$$$\"$\"\"\"\"       $o \"     $$ooooo$ $o\"\"$\"\"\"\"\"\"\"\"\"\"\"\"");
        System.out.println(" o$$\"$$$ooo$  $\" $$\"$$$$$$oooo$o$o\"\"\"\"$$\"\"o$oo$\"\"\"");
        System.out.println(" $$ o$$$$$$$  \"\" o$o$$$\"\"\"\"\"\"\" \" \"\"\"\"\"$$$$$o\"");
        System.out.println(" $o  $$$$\"\"\"\"$o$$$\"\"\"$o$\"\"\"\"\"\"\"\"\"\"\"\"\"\"$$");
        System.out.println("  \"\"o\"\"             $$o$\"\"\"o\"o oooooooo$oo");
        System.out.println("                    \"ooo$$oo oo o     o$\"");
        System.out.println("                    \"$$$$$$$$$$$$$$$$$$\"");
        System.out.println("            o$$\"\"\"\"\"$$$\"\"$$$$$$$$$$$$$$$oooooo");
        System.out.println("      o$  o$\"         \"$oo o o\"$$$$ $$\"$$\"\"\"\"$$o    o");
        System.out.println(" oo \"oo$$ $$o          \"\" \"\"\"  o$\" \"\"\"$        \"$$o$$  oo\"");
        System.out.println("\"     \"\"\"$\"\"$$$$o$ooooooooooo\"\"$oooo o o o       o$$\"\"\"\" \"$\"$\"");
        System.out.println("                                   \" \"\"\"\"\"\"\"$\"$\"$\"\"\"");
        System.out.println("Original Comic: http://www.gocomics.com/calvinandhobbes/1986/03/07");
    }
}
